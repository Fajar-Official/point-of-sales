        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <div>
                        <button @click="addForm('{{ route('members.store') }}')" class="btn btn-primary">
                            Tambah Member
                        </button>
                    </div>
...
@section('js')
    ...
    <script type="text/javascript">
        const apiUrl = "{{ route('members.api') }}";;
            ...
            methods: {
                ...
                handleAjaxError(error) {
                    if (error.response) {
                        ...
                        console.error('Error creating member. Server responded with:', error.response.status, error
                            .response.data);
                    } else if (error.request) {
                        // The request was made but no response was received
                        console.error('Error creating member. No response received from server.');
                    } else {
                ...
                editForm: function(id) {
                    const _this = this;
                    const updateUrl = `{{ route('members.update', ['member' => ':id']) }}`.replace(':id', id);

                    // Set modal title
                    $('#modal-form .modal-title').text('Edit Member');

                    // Fetch member data based on ID using Axios
                    axios.get(`{{ url('members') }}/${id}`)
                        .then((response) => {
                            const member = response.data;
                            console.log(response);
                            if (member) {
                                $('#modal-form [name=name]').val(member.name);
                                $('#modal-form [name=phone]').val(member.phone);
                                $('#modal-form [name=address]').val(member.address);
                                ...
                                alert('Gagal mengambil data member. Data tidak valid.');
                            }
                        })
                        .catch((error) => {
                            // Handle error
                ...
                deleteData(id) {
                    if (window.confirm('Are you sure you want to delete this member?')) {
                        const _this = this;
                        const deleteUrl = `{{ route('members.destroy', ['member' => ':id']) }}`.replace(':id',
                            id);
